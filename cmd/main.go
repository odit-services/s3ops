...
func init() {
	...
	utilruntime.Must(s3oditservicesv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	if err = (&controller.S3ServerReconciler{
		...
		setupLog.Error(err, "unable to create controller", "controller", "S3Server")
		os.Exit(1)
	}
	...
	if err = (&controller.S3UserReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
